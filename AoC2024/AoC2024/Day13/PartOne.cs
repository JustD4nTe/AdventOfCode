...
public class PartOne(string input) : Solution(input)
{
    public override long Solve()
    {
        ...
        foreach (var game in rawInput)
        {
            ...
            if (a <= 100 && b <= 100
                         && a * btnA[0] + b * btnB[0] == target[0]
                         && a * btnA[1] + b * btnB[1] == target[1])
            {
    ...
    private static int[] Parsexy(string str) // :3
        => str.Split(": ")[1].Split(", ").Select(x => int.Parse(x[2..])).ToArray();
}